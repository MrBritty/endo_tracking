...
{
	//string vidFile = "..\\sources\\testFile1_Aux.avi"; \*C:\\Users\\Britz\\Pictures\\1.png*\
	...
	// Experimenting for best segmentation

	src = imread("C:\\Users\\Britz\\Pictures\\1.png",1);

	//Mat hsv_range;
	//cvtColor(src, hsv_range,CV_BGR2HSV);
	//imshow(window_name,src);
	//setMouseCallback( window_name, onMouse, 0 );

	Mat hsv;
	Mat hist_channel[3];
	calculateHistogram(src, hist_channel); 
	

	cvtColor(src,hsv,CV_BGR2HSV);
	Mat channel[3], grayImg;

	//cvtColor(channel[0],grayImg, CV_BGR2GRAY);

	split(hsv, channel);
	Scalar m = mean(channel[0]);
    printf("%f\n", m[0]);
	//inRange(hsv, Scalar(100, 80, 50), Scalar(180, 180, 90), channel);
	namedWindow("Hue", WINDOW_NORMAL);
	imshow("Hue",channel[0]);
	resizeWindow("Hue", 640, 480);
	namedWindow("Saturation", WINDOW_NORMAL);
	imshow("Saturation",channel[1]);
	resizeWindow("Saturation", 640, 480);
	namedWindow("Value", WINDOW_NORMAL);
	imshow("Value",channel[2]);
	resizeWindow("Value", 640, 480);
